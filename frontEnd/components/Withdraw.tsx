...
export function Withdraw() {
  ...
  const [verifiedCertificateData, setVerifiedCertificateData] =
    useState<VerifiedCertificateDetails>();
  ...
  const {
    data: getCompanyData,
    isLoading: getCompanyDataIsLoading,
    isError: getCompanyDataIsError,
  } = useContractRead({
  ...
  const {
    data: createCertData,
    isLoading: createCertIsLoading,
    isError: createCertIsError,
    write: createCertWrite,
    isSuccess: Successfully,
  } = useContractWrite(CreateCertConfig);
  ...
  return (
    <Container className={clsx("pt-20 pb-16 lg:pt-32")}>
      {verifiedStatus && verifiedCertificateData && (
        <Verified
          open={isOpen}
          close={onClose}
          ...
          {contractAddresses?.map((contractAddress, index) => (
            ...
            >
              ...
              <div className="flex justify-between items-center">
                <p>{`${contractAddress.slice(0, 8)} ... ${contractAddress.slice(
                  -8
                )}`}</p>
                {selectedContract === contractAddress && (
